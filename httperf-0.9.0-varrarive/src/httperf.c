...
static struct option longopts[] =
{
    ...
    {"period",	     required_argument,	(int *) &param_period,	0},
    ...
    {"rate",	     required_argument,	(int *) &param_rate,		0},
...
main (int argc, char **argv)
{
    ...
    param.num_rates = 1;
    (param.rates[0]).num_conns = ((u_int64_t)(-1ULL));
    (param.rates[0]).dist = DETERMINISTIC;
#ifdef HAVE_SSL
    param.ssl_reuse = 1;
#endif
    ...
    while ((ch = getopt_long (argc, argv, "d:hvV", longopts, &longindex)) >= 0)
    {
        switch (ch)
        {
            case 0:
                ...
                else if (flag == &param_rate)
                {
                    errno = 0;
                    (param.rates[0]).rate_param = strtod (optarg, &end);
                    if (errno == ERANGE || end == optarg || *end)
                    {
                    ...
                    if ((param.rates[0]).rate_param <= 0.0)
                        (param.rates[0]).mean_iat = 0.0;
                    else
                        (param.rates[0]).mean_iat = 1/(param.rates[0]).rate_param;
                    (param.rates[0]).dist = DETERMINISTIC;
                    (param.rates[0]).num_conns = ((u_int64_t)(-1ULL));
                      /* set for maximum number of connections */
                }
                else if (flag == &param_period)	/* --period */
                {
                    int      retVal;
                    uint64_t num_conns;
                    char*    distinfo;

                    if (rate_index >= NUM_RATES - 1)
                    {
                        /* too many rates! */
                        fprintf(stderr, "Exceeded maximum number of rates %d\n", NUM_RATES);
                        exit(-1);
                    }
                    ...
                    if ('-' == *optarg)
                    {
                        ...
                        fprintf(stderr, "Distribution can't apply for negative number of connections!\n");
                        exit(1);
                    }

                    /* variable arrival rate */
                    retVal = sscanf(optarg, "%llu:%*s", &num_conns);

                    if (retVal == 1)
                    {
                        /* specified number of connections */
                        distinfo = (strstr(optarg, ":")) + 1;
                        if (((char*)(1)) == distinfo)
                        {
                            fprintf(stderr, "period option \"%s\" must delimit number of requests from distribution "
                                    " by colon.\n", optarg);
                            exit(1);
                        }
                    }
                    else
                    {
                        ...
                        num_conns = ((uint64_t)(-1ULL));
                        distinfo = optarg;
                    }

                    /* standard approach */
                    (param.rates[rate_index]).dist = DETERMINISTIC;
                    (param.rates[rate_index]).num_conns = num_conns;
                    if (!isdigit (*distinfo))
                        switch (tolower(*distinfo++))
                        {
                            case 'd':
                            {
                                (param.rates[rate_index]).dist = DETERMINISTIC;
                            ...
                            case 'u':
                            {
                                (param.rates[rate_index]).dist = UNIFORM;
                            }
                            break;

                            case 'e':
                            {
                                (param.rates[rate_index]).dist = EXPONENTIAL;
                            }
                            break;
                            ...
                            default:
                            {
                                fprintf (stderr, "%s: illegal interarrival distribution "
                                         "'%c' in %s\n",
                                         prog_name, distinfo[-1], distinfo - 1);
                                exit (1);
                            }
                        }
                    ...
                    errno = 0;
                    switch ((param.rates[rate_index]).dist)
                    {
                        case DETERMINISTIC:
                        case EXPONENTIAL:
                            (param.rates[rate_index]).mean_iat = strtod (distinfo, &end);
                            if (errno == ERANGE || end == distinfo || *end
                                || (param.rates[rate_index]).mean_iat < 0)
                            {
                        ...
                        case UNIFORM:
                            (param.rates[rate_index]).min_iat = strtod (distinfo, &end);
                            if (errno == ERANGE || end == distinfo
                                || (param.rates[rate_index]).min_iat < 0)
                            {
                            ...
                            distinfo = end + 1;
                            (param.rates[rate_index]).max_iat = strtod (distinfo, &end);
                            if (errno == ERANGE || end == distinfo || *end
                                || (param.rates[rate_index]).max_iat < 0)
                            {
                                fprintf (stderr, "%s: illegal request period %s\n",
                                         prog_name, distinfo);
                                exit (1);
                            }
                            (param.rates[rate_index]).mean_iat =
                                0.5 * ((param.rates[rate_index]).min_iat +
                                       (param.rates[rate_index]).max_iat);
                            break;
                    ...
                    (param.rates[rate_index]).rate_param =
                        ((param.rates[rate_index]).mean_iat <= 0.0)
                        ? 0.0 : (1.0 / (param.rates[rate_index]).mean_iat);
    ...
    if(0 != rate_index)
    {
        /* rate/period was specified */
        param.num_rates = rate_index + 1;
        (param.rates[rate_index]).num_conns = ((u_int64_t)(-1ULL));
        (param.rates[rate_index]).dist = DETERMINISTIC;
    }

    /* convert num_conns on rates to cumulative */
    for(idx = 1; idx < param.num_rates; idx++)
    {
        (param.rates[idx]).num_conns = (param.rates[idx]).num_conns +
            (param.rates[idx - 1]).num_conns;
    }
    ...
    for(idx = 0; idx < param.num_rates; idx++)
    {
        if ((param.rates[idx]).rate_param > 0.0)
        {
            switch ((param.rates[idx]).dist)
            {
                case DETERMINISTIC:
                    /* for backwards compatibility, continue to use --rate: */
                    printf (" --rate=%g", (param.rates[idx]).rate_param);
                    break;

                case UNIFORM:
                    printf (" --period=u%g,%g",
                            (param.rates[idx]).min_iat, (param.rates[idx]).max_iat);
                    break;

                case EXPONENTIAL:
                    printf (" --period=e%g", (param.rates[idx]).mean_iat);
                    break;
