...
recv_start (Event_Type et, Object *obj, Any_Type reg_arg, Any_Type call_arg)
{
  ...
  now = timer_now ();
  diff = now - c->basic.time_send_start;
  basic.call_response_sum += now - c->basic.time_send_start;
  basic.call_variance_sum += (now - c->basic.time_send_start)*(now - c->basic.time_send_start);
  if((now - c->basic.time_send_start) < basic.reply_time_min)
	basic.reply_time_min = now - c->basic.time_send_start;
  if((now - c->basic.time_send_start) > basic.reply_time_max)
	basic.reply_time_max = now - c->basic.time_send_start;
  c->basic.time_recv_start = now;
  ++basic.num_responses;
  printf("\nNum: %3d    ",basic.num_responses);
  printf("RT: %.1f Time: %f \n",1e3*(now-c->basic.time_send_start), now);
}
...
dump (void)
{
  ...
  printf ("Reply time [ms]: response %.1f max %.1f min %.1f transfer %.1f  variance sum %.1f\n",
	  1e3*resp_time, 1e3*basic.reply_time_max, 1e3*basic.reply_time_min, 1e3*xfer_time, 1e3*basic.call_variance_sum);
