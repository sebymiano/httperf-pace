...
static void
setup_next_rate_generator(Event_Type type, Object* obj, Any_Type arg)
{
    static u_int64_t num_conns = 0;

    num_conns++;
    /* increment number of connections seen */

    if(rg_sess.num_rates == 0)
    {
        /* no more rates, just continue using this one */
        return;
    }

    if(num_conns > ((rg_sess.rate)->num_conns))
    {
        /* switch over to next rate generator */
        rate_generator_stop(&rg_sess);
        rg_sess.rate++;
        rg_sess.num_rates--;
        rate_generator_start(&rg_sess, EV_CONN_DESTROYED);
    }
...
init (void)
{
    ...
    call_private_data_offset = object_expand (OBJ_CALL, sizeof (Call_Private_Data));
    sess_private_data_offset = object_expand (OBJ_SESS, sizeof (Sess_Private_Data));
    rg_sess.num_rates = param.num_rates - 1;
      /* first one is started here, so one less rate available */
    rg_sess.rate = param.rates;
...
start (void)
{
    ...
    event_register_handler(EV_CONN_NEW,
                           ((Event_Handler)(setup_next_rate_generator)),
                           ((Any_Type)(NULL)));
}
